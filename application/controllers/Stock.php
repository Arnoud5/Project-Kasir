...
class Stock extends CI_Controller {
	...
	{
		$data['row'] = $this->stock_m->get_stock_in()->result();
		$this->template->load('template', 'transaction/stock_in/stock_in_detail', $data);
	...
	public function stock_out_del()
	{
		...
		if($this->db->affected_rows() > 0) {
			$this->session->set_flashdata('success', 'Data Stock Keluar Berhasil Dihapus!');
		}
		redirect('stock/out');
	}
	...
	public function process_out() {
		if(isset($_POST['out_add'])) {
			...
			if($this->db->affected_rows() > 0) {
				$this->session->set_flashdata('success', 'Data Stock Keluar Berhasil Disimpan!');
			}
			redirect('stock/out');
		}
	}
	...
	{
		$data['row'] = $this->stock_m->get_stock_out()->result();
		$this->template->load('template', 'transaction/stock_out/stock_out_detail', $data);
	}
