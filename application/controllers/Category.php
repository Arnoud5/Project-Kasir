...
class Category extends CI_Controller {
	...
	public function process() 
	{
		...
		if(isset($_POST['add'])) {
			if($this->category_m->check_category($post['name'])->num_rows() > 0) {
				$this->session->set_flashdata('error', "Category $post[name] Sudah Terdaftar!");
				redirect('category/add');
			} else {
				$this->category_m->add($post);
			}
		} else if(isset($_POST['edit'])) {
			if($this->category_m->check_category($post['name'], $post['id'])->num_rows() > 0) {
				$this->session->set_flashdata('error', "Category $post[name] Sudah Terdaftar!");
